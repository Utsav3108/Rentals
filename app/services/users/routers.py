...
@router.delete("/user")
def delete_user_field(db : Session = Depends(get_db), current_user: User = Depends(get_current_user)):

    user = db.query(User).where(User.email == current_user.email).first()
